...
获取的奖励$R\_t$是一个随机变量。我们定义$q\_{*}(a)$为进行动作$a$的奖励期望：

$$
q_{*} = \mathbb{E}[R_t|A_t = a]
$$

为了求得该期望我们可以进行动作价值估计: $Q\_t(a) = q\_{*}(a)$。最基本的估计使用如下公式:
...
根据我们的估计期望$Q$，我们有贪婪算法$A\_t^{*} = \arg\max\_a Q\_t(a)$来选取最佳的动作。根据动作是否贪婪，我们可以将动作分成两部分：
1. 贪婪动作->采集
2. 非贪婪动作-> 探索
