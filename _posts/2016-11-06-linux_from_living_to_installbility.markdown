...
google,facebook,microsoft的高手，每次开战，少不了一场血雨腥风。

![](http://og78s5hbx.bkt.clouddn.com/zhugewang.jpg)

不过至少在我看来，我们这里倒是很少有人用vim或者emacs，刚刚入门的可能会用
...
这些操作，都可以在双手不离开键盘的情况下完成<del>双手打字，以示清白</del>。 如果你跟我一样是键盘党，就不难明白纯键盘操作比起鼠标操作是多么惬意的一
...
当你觉得vim的操作方式非常舒畅，想无论到哪里都用vim的话，还有这些插
件把你的生活环境vim化：
...
统的简单易用，而是觉得自己的脑子很simple,自己的智商很stupid。的确，
如果要真正发挥威力，linux不是一个对新手很好的系统。虽然有很多的文档，
社区与文章，linux入门确实不容易。但是，一旦理解了其原理，我们就能够发
现linux的简单易用之处--自动化的脚本，高效的编辑，合理的软件管理，安全
的权限控制，只要*你了解它*。

而且，除去桌面系统，linux可以说几乎应用在生活的方方面面。小至Android,
...
搞定个人的工作学习生活需求。虽然不是linux高手，但是在普通的
Windows/Mac用户<span class="heimu">妹子</span><span
class="heimu"><del>醒醒哪有妹子跟死宅说话</del></span>面前装逼还是绰绰有余。

<del>Windows:我先，明明是我先的……</del>
...
1. 虚拟机。虚拟机的好处是不会对原来系统造成损坏，但是坏处是资源占用高，
   没有了linux在各种硬件上都能高效运行的优势。另外，在虚拟机环境下，经
   常用一会儿就切回原来的系统去了。
2. 双系统。双系统的好处与坏处同虚拟机刚好相反，效率得到保证，而且可以
   进行沉浸式学习。但是装双系统稍微有点复杂（其实如ubuntu之类已经很简
   单了）,新手有玩儿脱的可能，而且双系统的系统恢复对新手而言也不容易。

常用的虚拟机有virtual box, vmware。但是前者是免费的，后者更注重专业应
用。

要安装双系统的话，windows下有一个很好用的软件：
...
的麻烦。每个发行版通常会带有一个软件包管理器，以方便安装维护软件。不像
Windows与Mac版本一条线跑到黑，Linux发行版可谓琳琅满目。
